...
export default function DashboardPage() {
  ...
  // Warm the cache on mount (and when start/end changes)
  useEffect(() => {
    let mounted = true;
    async function warm() {
      setWarming(true);
      try {
        ...
        console.warn('warmCache failed', err);
      } finally {
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-3">
          <button
            className="px-3 py-2 bg-white text-slate-800 rounded shadow cursor-pointer"
            onClick={() => { setStart(DEFAULT_START); setEnd(DEFAULT_END); }}
          >
            Reset window
          </button>
          ...
          <button
            ...
            className="px-4 py-2 bg-indigo-600 text-white rounded shadow disabled:opacity-60 cursor-pointer"
          >
      ...
      <section className="mb-6">
        <SummaryCards metrics={factoryMetrics ?? undefined} loading={factoryLoading} />
